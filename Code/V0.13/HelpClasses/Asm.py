...
class Asm:
    ...
    def __init__(self):
        self.function_offsets = {}  # Maps 'func_name' -> absolute byte index
    ...
    def emit_stack_offset_write(self, index, src_reg):
        """
        Encodes a 'mov [rsp + index], src_reg' instruction into raw machine code bytes.
        Supports 64, 32, and 16-bit register sources.
        Maps to Dia syntax: ^:[index]src_reg
        """
        src = src_reg.lower().strip()

        if src not in self.REG_MATRIX:
            raise ValueError(f"Unsupported or unmapped source register: {src_reg}")

        src_size, src_idx = self.REG_MATRIX[src]
        out_bytes = bytearray()

        # 1. Handle 16-bit Size Prefix Override (0x66)
        if src_size == 16:
            out_bytes.append(0x66)

        # 2. Determine Displacement Mode Configuration
        # Mod bits (Bits 6-7 of ModR/M):
        # 0x01 = 8-bit signed displacement (-128 to 127)
        # 0x02 = 32-bit signed displacement
        # Note: Even if index is 0, we use Mod 0x01 with a [rsp + 0] byte to avoid SIB base-elimination traps.
        if -128 <= index <= 127:
            mod = 0x01
            is_disp8 = True
        else:
        ...
        # 3. Calculate REX Prefix dynamically
        # REX layout: 0100 W R X B
        # Bit 3 (W) = 1 for 64-bit source register size
        # Bit 2 (R) = 1 if source register index >= 8 (Extends ModR/M 'reg' field)
        # Bit 0 (B) = 0 because base register is RSP (index 4), which is less than 8
        rex_byte = 0x40
        is_rex_needed = False
        ...
        if is_rex_needed:
            out_bytes.append(rex_byte)

        # 4. Append Core Opcode (0x89 is MOV r/m, reg)
        out_bytes.append(0x89)

        # 5. Construct ModR/M Byte
        # Mode = mod (0x01 or 0x02)
        # Reg  = lower 3 bits of source register index (src_idx & 7)
        # R/M  = 4 (An R/M value of 4 explicitly commands the CPU to read a SIB byte)
        modrm_byte = (mod << 6) | ((src_idx & 7) << 3) | 4
        out_bytes.append(modrm_byte)

        # 6. Construct SIB Byte (Required when RSP is the base anchor)
        # Scale = 00 (No hardware scaling offset)
        # Index = 100 (4 = No index register used)
        # Base  = 100 (4 = RSP is the hardware base register)
        sib_byte = (0x00 << 6) | (4 << 3) | 4
        out_bytes.append(sib_byte)

        # 7. Append the Displacement payload
        if is_disp8:
            out_bytes.append(index & 0xFF)
        else:
            for i in range(4):
                out_bytes.append((index >> (i * 8)) & 0xFF)

        # 8. Commit and Return
        self.code.extend(out_bytes)
        return out_bytes

    def emit_stack_offset_read(self, index, dest_reg):
        """
        Encodes a 'mov dest_reg, qword ptr [rsp + index]' instruction into raw bytes.
        Supports 64, 32, and 16-bit destination registers.
        Maps to Dia syntax: v:([index]->dest_reg)
        """
        dest = dest_reg.lower().strip()

        if dest not in self.REG_MATRIX:
            raise ValueError(f"Unsupported or unmapped destination register: {dest_reg}")

        dest_size, dest_idx = self.REG_MATRIX[dest]
        out_bytes = bytearray()
        ...
        # 3. Calculate REX Prefix dynamically
        # REX layout: 0100 W R X B
        # Bit 3 (W) = 1 for 64-bit destination register size
        # Bit 2 (R) = 1 if destination register index >= 8 (Extends ModR/M 'reg' field)
        # Bit 0 (B) = 0 because the base register is RSP (index 4), which is less than 8
        rex_byte = 0x40
        is_rex_needed = False
        ...
        # 4. Append Core Opcode (0x8B is MOV reg, r/m)
        out_bytes.append(0x8B)

        # 5. Construct ModR/M Byte
        # Mode = mod (0x01 or 0x02)
        # Reg  = lower 3 bits of destination register index (dest_idx & 7)
        # R/M  = 4 (An R/M value of 4 explicitly commands the CPU to read a SIB byte)
        modrm_byte = (mod << 6) | ((dest_idx & 7) << 3) | 4
        out_bytes.append(modrm_byte)
        ...
        # 8. Commit to your compiler's byte stream and return
        self.code.extend(out_bytes)
        return out_bytes

    def emit_for_prologue(self):
        """
        Marks the beginning anchor point of a FOR loop iteration sequence and
        emits a conditional near-relative exit jump (JGE) with placeholder bytes.
        Returns: (loop_start_pos, jmp_exit_patch_pos)
        """
        # 1. Capture the absolute address offset where the loop evaluation cycle begins
        loop_start_pos = len(self.code)

        # 2. Emit standard JGE near-jump opcode (0x0F 0x8D) with 4 empty displacement bytes
        self.code.extend([0x0F, 0x8D, 0x00, 0x00, 0x00, 0x00])

        # 3. Pinpoint the starting index of the 32-bit placeholder array for late-patching
        jmp_exit_patch_pos = len(self.code) - 4

        return loop_start_pos, jmp_exit_patch_pos

    def emit_for_epilogue(self, loop_start_pos, jmp_exit_patch_pos):
        """
        Emits an unconditional near JMP back to the loop prologue anchor, then
        rewinds and patches the exit jump placeholder with the final forward displacement.
        """
        # 1. Append the unconditional near JMP opcode (0xE9)
        self.code.append(0xE9)

        # 2. Compute the relative backward step distance
        # Calculation must be relative to the instruction following the jump target data block
        next_inst_addr = len(self.code) + 4
        jmp_back_target = loop_start_pos - next_inst_addr

        # 3. Pack and append the signed 32-bit backward relative address step
        for i in range(4):
            self.code.append((jmp_back_target >> (i * 8)) & 0xFF)

        # 4. Resolve the forward exit target boundaries
        loop_end_pos = len(self.code)
        exit_next_inst_addr = jmp_exit_patch_pos + 4
        jmp_exit_target = loop_end_pos - exit_next_inst_addr

        # 5. Hot-patch the forward placeholder bytes directly within the bytearray memory
        for i in range(4):
            self.code[jmp_exit_patch_pos + i] = (jmp_exit_target >> (i * 8)) & 0xFF

    def compile_while_prologue(self, operator_string):
        """
        Maps a high-level operator string to its corresponding x64 short jump byte,
        then emits the instruction and a placeholder offset.
        """
        # Dictionary mapping the conditional operators to their inverse/exit opcodes
        # Note: We use the INVERSE jump because if the condition is FALSE, we want to jump OUT of the loop.
        jumps = {
            '==': 0x75,  # JNE (Jump if Not Equal / Not Zero) -> exits if rax != rbx
            '!=': 0x74,  # JE  (Jump if Equal / Zero) -> exits if rax == rbx
            '<': 0x7D,  # JGE (Jump if Greater or Equal, signed) -> exits if rax >= rbx
            '>': 0x7C,  # JL  (Jump if Less, signed) -> exits if rax <= rbx
            '<=': 0x7F,  # JG  (Jump if Greater, signed) -> exits if rax > rbx
            '>=': 0x7E,  # JLE (Jump if Less or Equal, signed) -> exits if rax < rbx
        }

        if operator_string not in jumps:
            raise ValueError(f"Unsupported loop operator: {operator_string}")

        jump_opcode = jumps[operator_string]

        # Emit the conditional jump opcode and the 1-byte displacement placeholder
        cond_jump_idx = len(self.code)
        self.code.extend([jump_opcode, 0x00])

        return cond_jump_idx

    def compile_while_epilogue(self, loop_start_target, cond_jump_idx):
        """
        Emits the backward jump (0xEB) to the loop start,
        then reaches back to patch the prologue's forward jump placeholder.
        """
        # 1. Emit Unconditional Jump Backward (0xEB) to the loop start target
        jmp_back_idx = len(self.code)
        backward_offset = loop_start_target - (jmp_back_idx + 2)
        self.code.extend([0xEB, backward_offset & 0xFF])

        # 2. Patch the Prologue's Conditional Jump Forward offset slot
        post_loop_idx = len(self.code)
        forward_offset = post_loop_idx - (cond_jump_idx + 2)

        if forward_offset > 127:
            raise ValueError(f"Loop body layout ({forward_offset} bytes) exceeds x64 short jump limits.")

        # Overwrite the 0x00 placeholder byte with the calculated offset
        self.code[cond_jump_idx + 1] = forward_offset & 0xFF

    def compile_function_prologue(self, name):
        # Log its relative position inside the dedicated function buffer
        self.function_offsets[name] = len(self.functions)

        # push rbp, mov rbp, rsp
        self.emit_byte([0x55, 0x48, 0x89, 0xE5])
    def compile_function_epilogue(self,):
        """
        Tears down the stack frame and emits the hardware return instruction.
        """
        # 3. Emit the standard x64 Epilogue:
        # pop rbp           -> 0x5D
        # ret               -> 0xC3
        self.code.extend([0x5D, 0xC3])

    def end(self):
        main_code_size = len(self.code)

        for func in self.function_offsets:
            self.function_offsets[func] += main_code_size

        # Glue the function text section directly to the end of the main text section
        final_executable_binary = self.code + self.functions
        return final_executable_binary

    def compile_function_call(self, node):
        """
        node layout: ('func_call', 'func_name')
        """
        func_name = node

        # 1. Emit the Call Opcode (0xE8)
        self.emit_byte(0xE8)

        # 2. Record the exact index of the 4-byte displacement placeholder
        placeholder_idx = len(self.code)

        # 3. Emit 4 bytes of zero placeholders (0x00, 0x00, 0x00, 0x00)
        self.emit_byte([0x00, 0x00, 0x00, 0x00])

        # 4. Save this site to our relocation table to fix later
        self.call_relocations.append((placeholder_idx, func_name))

    def finalize_program(self):
        # 1. Capture the exact final boundary of the main script array
        final_main_size = len(self.code)

        # 2. Shift your function dictionary addresses to account for their new position at the end
        for func in self.function_offsets:
            # If your function local index was 0, it properly shifts to 0 + 15 = 15 (0x0F)
            self.function_offsets[func] += final_main_size

        # 3. Flatten the reservoirs into your final mutable binary list
        final_binary = list(self.code + self.functions)

        # 4. Patch your relocation points
        for placeholder_idx, func_name in self.call_relocations:
            target_address = self.function_offsets[func_name]

            # Calculate the 32-bit displacement relative to the end of the call instruction
            relative_offset = target_address - (placeholder_idx + 4)

            # Slice the 4 signed little-endian bytes into place
            offset_bytes = (relative_offset & 0xFFFFFFFF).to_bytes(4, byteorder='little')

            final_binary[placeholder_idx:placeholder_idx + 4] = offset_bytes

        return bytes(final_binary)
    ...
    def emit_byte(self,value):
        if isinstance(value, (list, tuple, bytes, bytearray)):
            ...
            # Casts to a standard 8-bit unsigned integer to prevent overflows
            self.code.append(value & 0xFF)

    def write_exe(self, filename):
        payload = self.code + self.functions

        # 1. Align the machine code payload to 512-byte disk sectors
        payload_padding = (512 - (len(payload) % 512)) % 512
        padded_payload = payload + b'\x00' * payload_padding

        size_of_raw_data = len(padded_payload)
        virtual_size = len(payload)
        size_of_image = (0x1000 + virtual_size + 4095) & ~4095

        # 2. Create a fresh 512-byte header container
        header = bytearray(512)

        # --- DOS Stub Header ---
        struct.pack_into('<2s', header, 0x00, b'MZ')
        struct.pack_into('<I', header, 0x3C, 0x40)  # PE Signature pointer points to 0x40

        # --- PE Signature (4 bytes) ---
        struct.pack_into('<4s', header, 0x40, b'PE\x00\x00')

        # --- COFF File Header (20 bytes) ---
        # Machine=x64 (0x8664), Sections=1, Time=0, PointerToSymbolTable=0, NumberOfSymbols=0, SizeOfOptionalHeader=240 (0xF0), Characteristics=0x0022
        struct.pack_into('<HHIIIHH', header, 0x44, 0x8664, 1, 0, 0, 0, 240, 0x0022)

        # --- Optional Header (240 bytes total) ---
        # Standard Fields (24 bytes)
        # Magic=PE32+ (0x020B), MajorLinker=1, MinorLinker=0, SizeOfCode, SizeOfInitData=0, SizeOfUninitData=0, AddressOfEntryPoint=0x1000, BaseOfCode=0x1000
        struct.pack_into('<HBBIIIII', header, 0x58, 0x020B, 1, 0, size_of_raw_data, 0, 0, 0x1000, 0x1000)

        # Windows-Specific Fields (88 bytes)
        # ImageBase=0x400000, SectionAlignment=0x1000, FileAlignment=0x0200, MajorOS=10, MinorOS=0, MajorImage=0, MinorImage=0, MajorSubsystem=10, MinorSubsystem=0, Win32VersionValue=0
        struct.pack_into('<QQIIHHHHHH', header, 0x70, 0x400000, 0x1000, 0x0200, 10, 0, 0, 0, 10, 0, 0)
        # SizeOfImage, SizeOfHeaders=512, CheckSum=0, Subsystem=3 (Console), DllCharacteristics=0, StackReserve=1MB, StackCommit=4KB, HeapReserve=1MB, HeapCommit=4KB
        struct.pack_into('<IIIIHQQQQ', header, 0x90, size_of_image, 512, 0, 3, 0, 0x100000, 0x1000, 0x100000, 0x1000)
        # LoaderFlags=0, NumberOfRvaAndSizes=16
        struct.pack_into('<II', header, 0xC4, 0, 16)

        # Data Directories Space (128 bytes)
        # Starts at 0xCC and runs up to exactly 0x14C. Left clean as 0x00.

        # --- Section Table Descriptor: .text (40 bytes) ---
        # MUST start exactly at 0x14C (0x58 + 240) right where the Optional Header ends!
        struct.pack_into('<8sIIIIIIHHI', header, 0x14C,
                         b'.text\x00\x00\x00',
                         virtual_size,
                         0x1000,  # VirtualAddress (RVA)
                         size_of_raw_data,
                         0x0200,  # PointerToRawData (Points to byte 512 on disk)
                         0, 0, 0, 0,
                         0x60000020)  # Flags: CODE | EXECUTE | READ

        # 3. Stitch header container cleanly to the padded payload
        final_binary = header + padded_payload
        with open(filename, 'wb') as f:
            f.write(final_binary)

        print(f"[+] Executable built with exact 0x14C layout alignment: {filename}")